...
layout = [
    ...
    [
        [
                        ...
                        sg.Column(
                            [
                                [
                                    sg.Text(
                                        "현재 지진 정보가 없습니다.",
                                        font=recenteqregiont,
                                        key="eqregion" + str(i + 2),
                                        text_color="grey90",
                                        background_color="grey30",
                                        pad=(0, 0),
                                    )
            ...
            [sg.Column((), size=(0, 2), background_color="grey23")],
        ]
...
window = sg.Window(
    "Closing the Door", layout, resizable=True, background_color="grey23"
)
...
def soundplay(music):
    t = threading.Thread(
        target=playsound.playsound, args=("./audio/" + music + ".wav",)
    )
    t.start()
...
def guiupdate():
    for i in range(listlength):
        if (eqsource[i] == "PEWS" and int(eqsin[i]) < 5) or (
            ...
            fontcolor1 = "white"
        if (eqsource[i] == "PEWS" and int(eqsin[i]) > 10) or (
            eqsource[i] != "PEWS" and eqsin[i] not in sindoromaji
        ):
            fontcolor1 = "white"
            colcolor1 = "black"
        ...
        if os.path.isfile("bin/isbroadcast"):
            window["eqsec" + str(i + 1)].update("")
        else:
...
def kmaeqkparse(first):
    ...
    if first == True:
        url = (
            "https://www.weather.go.kr/w/eqk-vol/search/korea.do?schOption=&xls=0&startTm=2023-12-01&endTm="
            + datetime.datetime.now().strftime("%Y-%m-%d")
            + "&startSize=&endSize=&startLat=&endLat=&startLon=&endLon=&lat=&lon=&dist=&keyword=&dpType=a"
        )
    else:
        url = (
            "https://www.weather.go.kr/w/eqk-vol/search/korea.do?schOption=&xls=0&startTm=2023-12-01&endTm="
    ...
    if first == True:
        ...
        for row in row2s[:1]:
            ...
            diffnuridata = (columns[0].text.strip())
            print(diffnuridata)
        ...
        lastnuridata = str(int(columns[0].text.strip()) - int(diffnuridata) + 5)
    else:
        for row in rows[:1]:  # 첫 번째에서 첫 번째까지의 행만 추출
            ...
            if lastnuridata == columns[0].text.strip() or (
                ...
                and columns[2].text.strip() == eqsize[eqsource.index("weathergokr")]
                and columns[3].text.strip() == eqdepth[eqsource.index("weathergokr")]
...
def handle_eqk(body, info_bytes, phase):
    ...
    eqk_max_area = [
        AreaNames[i] for i, bit in enumerate(eqk_max_area_str) if bit == "1"
    ]

    url2 = (
        f"https://www.weather.go.kr/pews/data/{eqk_id}"  # 지진 ID를 통한 발생위치 정보 수신을 위한 url
    )
    ...
    if eqk_id in eqid:  # 지진이 포함된 경우 (업데이트)
        if (
            orig_lat not in eqlat
            or orig_lon not in eqlon
            or eqk_mag not in eqsize
            or eqk_dep not in eqdepth
            or datetime.datetime.fromtimestamp(eqk_time + 9 * 3600).strftime(
                "%Y/%m/%d %H:%M:%S"
            )
            not in eqtime
            or eqk_max not in eqsin
        ):  # 변경 내용이 있는 경우에만
            ...
            eqtime[eqid.index(eqk_id)] = datetime.datetime.fromtimestamp(
                eqk_time + 9 * 3600
            ).strftime("%Y/%m/%d %H:%M:%S")
            eqsin[eqid.index(eqk_id)] = eqk_max
            eqid[eqid.index(eqk_id)] = eqk_id
            eqrectime[eqid.index(eqk_id)] = (
                datetime.datetime.now().strftime("%H:%M:%S") + " 수신"
            )
            if eqsec[eqid.index(eqk_id)] > 0:
                eqsec[eqid.index(eqk_id)] = int(
                    format(
                        haversine((eqlat, eqlon), curpos, unit="km") / 3.75
        ...
        eqtime.insert(
            0,
            datetime.datetime.fromtimestamp(eqk_time + 9 * 3600).strftime(
                "%Y/%m/%d %H:%M:%S"
            ),
        )
    ...
    print("깊이:", eqk_dep)
    print(
        "시각:", datetime.datetime.fromtimestamp(eqk_time).strftime("%Y-%m-%d %H:%M:%S")
    )
    print("ID:", eqk_id)
    print("설명:", eqk_str)
    print("최대 진도:", eqk_max)
...
def handle_stn(stn_body, bin_body):
    stn_lat = [
        30 + int(stn_body[i : i + 10], 2) / 100 for i in range(0, len(stn_body), 20)
    ]
    stn_lon = [
        120 + int(stn_body[i + 10 : i + 20], 2) / 100
        for i in range(0, len(stn_body), 20)
    ]
...
def handlecomm():
    ...
    try:
        bin_time = (
            datetime.datetime.utcnow() - datetime.timedelta(milliseconds=tide)
        ).strftime("%Y%m%d%H%M%S")
        ...
        if errorhappened == False:
            ...
            if datetime.datetime.utcnow() >= next_sync_time:
                next_sync_time = datetime.datetime.utcnow() + datetime.timedelta(
                    seconds=10.0
                )
            ...
            if bytes_data and len(bytes_data) > MaxEqkStrLen:
                header = "".join(
                    [byte_to_bin_str(byte_val) for byte_val in bytes_data[:HeadLength]]
                )
                body = byte_to_bin_str(bytes_data[0]) + "".join(
                    [byte_to_bin_str(byte_val) for byte_val in bytes_data[HeadLength:]]
                )
                ...
                if StationUpdate:
                    ...
                    stn_body = "".join(
                        [byte_to_bin_str(byte_val) for byte_val in stn_bytes]
                    )
